...
from dsets.wfd import ScriptlevelDataset
from experiments.py.eval_utils_counterfact import compute_rewrite_quality_counterfact
from experiments.py.eval_utils_wikifactdiff import compute_rewrite_quality_wikifactdiff_functional,apply_edits
...
DS_DICT = {
    ...
    "script": (ScriptlevelDataset, compute_rewrite_quality_wikifactdiff_functional)
}
...
def main(
    ...
    ds = ds_class(globals.DATA_DIR, tok=tok, size=dataset_size_limit, path=dataset_path, use_random_neighbors=use_random_neighbors,start = dataset_start)
    ...
    for chunk_pos, record_chunks in enumerate(chunks(ds, num_edits), start=offset):
        ...
        pre_metrics = {}
        if not skip_pre_metrics:
            with torch.no_grad():
                copy = apply_edits(model, weights_copy)
                
                for record in record_chunks:
                    pre_metrics[record['case_id']] = ds_eval_method(
                        model,
                        tok,
                        record,
        ...
        for record in record_chunks:
            ...
            try:
                metrics = {
                    ...
                    "failed" : failed,
                    "pre": pre_metrics[record['case_id']] if not skip_pre_metrics else None,
                }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ds_name",
        choices=["mcf", "cf", "zsre", "wfd","script"],
        default="wfd",
    ...
    parser.add_argument(
        "--skip_pre_metrics",
        dest="skip_pre_metrics",
        action="store_true",
        help="Skip pre-metric computation",
    )    
    parser.add_argument(
        "--dataset_start",
        type=int,
        default=0,
        help="Start from the dataset_start-th instance of the dataset",
    )
    ...
    main(
        ...
        offset=args.offset,
        skip_pre_metrics=args.skip_pre_metrics,
