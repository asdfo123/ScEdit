...
class PromptedModel():
    ...
    def forward(self, input_ids : torch.Tensor, attention_mask, **kwargs):
        ...
        for i, x in enumerate(input_ids):
            if 'llama' in self.model.config._name_or_path.lower():
                inp = self.prompt + self.tokenizer.decode(x[:lengths[i]],skip_special_tokens = True)
            else:
    ...
    def generate(self, input_ids, attention_mask, **kwargs):
        ...
        for x in input_ids:
            inp = self.prompt + self.tokenizer.decode(x, skip_special_tokens = True).strip(self.tokenizer.pad_token)
            print("inp: "+ inp)
            inputs_.append(inp)
